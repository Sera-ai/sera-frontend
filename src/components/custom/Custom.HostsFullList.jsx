            ...
            <thead className="text-xs uppercase text-slate-400 dark:text-slate-500 bg-slate-50 mainDark dark:bg-opacity-50 rounded-sm sticky top-0">
              <tr className='pl-4'>
                {Object.keys(apiInventory[0]).map((key, index) => {
                  return (<th className={`p-2 ${index == 0 && " pl-8"}`}>
                  ...
                  <tr className="rowItemLight" style={{ backgroundColor: "#ffffff05" }}>
                    <td className="p-2 pl-8">
                      <div className="flex flex-col">
                        <a href={`/editor/${filteredData[data].host}/`} className="text-slate-800 dark:text-slate-100">{filteredData[data].host}</a>
                      </div>
                    </td>
                    <td className="p-2">
                      <div className="flex flex-row items-center space-x-2.5">
                        ...
                        {/* Percentage */}
                        <div className="text-center text-xs">{Math.round(filteredData[data].documentation * 100)}%</div>
                      </div>
                    </td>
                    <td className="p-2">
                      <div className="text-center">{(filteredData[data].requests).toLocaleString()}</div>
                    </td>
                    <td className="p-2">
                      <div className="text-center">{filteredData[data].endpoints}</div>
                    </td>
                    <td></td>
                  </tr>
                </React.Fragment>
