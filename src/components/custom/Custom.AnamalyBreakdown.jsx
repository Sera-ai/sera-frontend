...
function AnamalyBreakdown({issueListChart}) {
  return (
    <div className="col-span-full xl:col-span-6  mainDark shadow-lg rounded-sm border border-slate-200 dark:border-slate-700">
      ...
      <div className="px-5 py-3">
        <div className="flex items-start">
          <div className="text-3xl font-bold text-slate-800 dark:text-slate-100 mr-2">
            {issueListChart.datasets?.length}
          </div>
        </div>
      </div>
      ...
      <div className="grow">
        ...
        {Object.keys(issueListChart).length > 0 && (
          <BarChart data={issueListChart} width={595} height={48} />
        )}
      </div>
    </div>
  );
