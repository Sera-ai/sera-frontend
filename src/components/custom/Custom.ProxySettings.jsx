...
function ProxySettings({ endpoint }) {
    const [selected, setSelected] = useState('SERA Endpoint');
    const { endpointProxy } = useContext(AppContext)
    ...
    const options = [
        {
            name: 'SERA Endpoint', domain: endpointProxy[endpoint].seraUrl, icon: (<svg xmlns="http://www.w3.org/2000/svg" width="17" height="18" viewBox="0 0 17 18" fill="none">
    ...
    return (
        <div className="py-4 rounded-md">
            {options.map((option, index) => (
                <div
                    key={index}
                    className={`flex items-center py-2 px-3 ${index == 0 ? "border-t border-l border-r rounded-t-lg" : index == (options.length - 1) ? "rounded-b-lg border-b border-l border-r" : "border"} border-slate-200 dark:border-slate-700 cursor-pointer`}
                    onClick={() => setSelected(option.name)}
                >
                    <div style={{ backgroundColor: selected === option.name ? "#2B84EC" : "#1A1C20" }} className={`p-2.5 mr-4 rounded-md`}>{option.icon}</div>
                    ...
                    <div>
                        {selected === option.name ? (
                            <div className="secondaryDark w-5 h-5 rounded-full flex justify-center items-center">
                                <div style={{ backgroundColor: "#2B84EC" }} className="w-3 h-3 rounded-full"></div>
                            </div>
                        ) : (
