...
function Ecosystem() {
    ...
    const [endpoint, setEndpoint] = useState("/catalog/api.sample.com/pets/__post");
    ...
    useEffect(() => {
        ...
        if (network) {
            ...
            network.on("zoom", function (params) {
                ...
                nodes.forEach(node => {
                    network.body.data.nodes.update({ id: node.id, font: { size: newFontSize } });
                });
            });
        }
    ...
    const graphOptions = {
        nodes: {
            ...
            color: {
                ...
                highlight: {            // Color when the node is highlighted (selected or hovered)
                    border: '#2B7CE9',
                    background: '#D2E5FF'
                },
                hover: {                // Color when the node is hovered
                    border: '#2B7CE9',
                    background: '#D2E5FF'
                },
    ...
    const tableData = (data, index, total) => {
        ...
        if (index == 0) {
            return (<span
                ...
                style={data.toLowerCase() === 'active' ? { backgroundColor: '#23A85830', color: "#23A858" } : { backgroundColor: '#EC2B2B30', color: "#EC2B2B" }}
            >
                {returnData}
            </span>)
    ...
    return (
        <div className="flex h-screen overflow-hidden">
            ...
            <div className="relative flex flex-col flex-1 w-full items-center">
                <Header sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} title={"Ecosystem"} />
                <main className="flex flex-col items-center w-full" style={{ height: "-webkit-fill-available" }}>
                    <div className="flex justify-end gap-2 z-10 " style={{ width: '1220px' }}>
                        {/* Filter button */}

                        <FilterButton />
                        {/* Datepicker built with flatpickr */}
                        <form style={{ borderColor: "#334155" }} className="mainDark text-slate-500 hover:text-slate-600 dark:text-slate-300 dark:hover:text-slate-200 font-medium w-[15.5rem] rounded-sm border py-0">
                    ...
                    <div className="flex-grow overflow-visible">
                        <div style={{ position: "absolute", top: -300, right: 0, bottom: 0, left: -100, zIndex: 1 }}>
                            <Graph
                                graph={{
                                    nodes: nodess,
                                    edges: edgess
                    ...
                    <div className='w-full z-10'>
                        ...
                        <div style={{ height: 400, borderRadius: 5, padding: 10 }} className='w-full mainDark'>
                            <div style={{ borderRadius: 8 }} className='h-full secondaryDark'>
                                <div className="flex flex-column space-x-10 w-full h-full ">
                                    <div style={{ borderRightWidth: 1, borderColor: "#ffffff30", borderTopLeftRadius: 5 }} className='text-sm py-3 h-full flex flex-col secondaryDark'>
                                    ...
                                    <div className="flex flex-column space-x-10 w-full h-full py-14 px-5">
                                                            ...
                                                            <tr className="secondaryDark rowItem noborder">
                                                                ...
                                                                <td className="p-1">
                                                                    <div className="flex flex-wrap gap-2">
                                                                        <div className="text-slate-800 text-xs dark:text-slate-100 flex flex-wrap">{tableData(endpointDetails[endpoint]["details"][obj], index, Object.keys(endpointDetails[endpoint]["details"]).length)}</div>
                                                                    </div>
                                                                </td>
                                                            </tr>
                                                        </React.Fragment>
                                        ...
                                        <div className=" w-full">
                                            ...
                                            <h2 style={{ color: "#ffffff99" }} className="text-xs text-slate-800 dark:text-slate-100">Statistics available for this endpoint</h2>
                                                ...
                                                <tbody className="text-sm font-medium divide-y divide-slate-100 dark:divide-slate-700">
                                                    {Object.keys(endpointDetails[endpoint]["statistics"]).map((obj, index) => (
                                                        <tr key={index} className="secondaryDark rowItem noborder">
                                                            ...
                                                            <td className="p-1">
                                                                <div className="flex flex-wrap gap-2">
                                                                    <div className="text-slate-800 text-xs dark:text-slate-100 flex flex-wrap">{endpointDetails[endpoint]["statistics"][obj]}</div>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                    ))}
...
const HeaderTabs = ({ selectedTab, setSelectedTab }) => {
    ...
    return (
        <div className="text-white flex full-w ml-3 text-sm">
            {/* Tab: All Coins */}
            <div
                className={`cursor-pointer py-1 ${selectedTab === 0 ? 'border-b-2 border-blue-300 dark:text-slate-100' : 'dark:text-slate-500'}`}
            ...
            {/* Tab: Portfolio */}
            <div
                className={`cursor-pointer py-1 ml-6 ${selectedTab === 1 ? 'border-b-2 border-blue-300 dark:text-slate-100' : 'dark:text-slate-500'}`}
                style={{ borderColor: "#2b84ec" }}
